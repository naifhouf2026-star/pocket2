...
class AsyncPocketOptionClient:
    ...
    def __init__(
        ...
        self._last_live_candle: Optional[Candle] = None
        ...
        if isinstance(data, dict) and "assets" in data:
            for asset, info in data["assets"].items():
                ...
                if payout != self._payout_cache.get(asset):
                    ...
                    #print(f"Updated payout cache for {asset}: {payout}")
    ...
    async def get_candles_live3(self, asset: str, timeframe: Union[str, int]) -> Optional[Dict[str, Any]]:
            """
            دالة المراقبة: تقرأ السعر اللحظي مباشرة من الكاش (بدون إرسال طلبات متكررة للسيرفر)
            """
            ...
            if cache_key in self._candles_cache and len(self._candles_cache[cache_key]) > 0:
                c = self._candles_cache[cache_key][-1]
                return {
    ...
    async def get_candles(
        ...
        for attempt in range(max_retries):
            try:
                # 1. طلب الشموع القديمة (بيعمل resolve للـ Future)
                ...
                if cache_key not in self._candles_cache:
                    ...
                    # هنعمل دمج (Merge) عشان نحافظ على شموع الـ history والـ ticks اللي وصلت
                    merged_dict = {c.timestamp: c for c in candles}
                    
                    for c in self._candles_cache[cache_key]:
                        # الكاش بيكسب لو فيه تعارض لأن التحديث اللحظي فيه بيكون أدق
                        merged_dict[c.timestamp] = c
                        
                    # نرجعها قائمة مرتبة زمنياً
                    self._candles_cache[cache_key] = sorted(list(merged_dict.values()), key=lambda x: x.timestamp)

                # 3. ننتظر نص ثانية عشان رسالة الـ history (الشمعة الحالية) توصل للكاش
                await asyncio.sleep(0.5)

                # 4. نسحب الداتا النهائية من الكاش (الشمعة القديمة + الحالية)
                final_candles = self._candles_cache.get(cache_key, candles)

                logger.info(f"Retrieved {len(final_candles)} candles for {asset} (including live candle)")
    ...
    async def get_candles_live(self, asset: str, timeframe: Union[str, int]) -> Optional[Dict[str, Any]]:
        """
        يرجع آخر شمعة لحظية تم تسجيلها من السيرفر كـ Dictionary
        """
        if isinstance(timeframe, str):
            timeframe_seconds = TIMEFRAMES.get(timeframe, 60)
        else:
        ...
        # نتأكد إننا باعتين طلب تتبع للسيرفر
        live_data = {"asset": str(asset), "period": timeframe_seconds}
        live_msg = f'42{json.dumps(["changeSymbol", live_data])}'
        
        if self._is_persistent and self._keep_alive_manager:
            await self._keep_alive_manager.send_message(live_msg)
        else:
            await self._websocket.send_message(live_msg)

        # ننتظر لحظة عشان السيرفر يلحق يبعت داتا
        await asyncio.sleep(0.5)

        # لو في شمعة متسجلة، نرجعها بشكل أنيق
        if self._last_live_candle:
            c = self._last_live_candle
            return {
                "timestamp": c.timestamp.strftime('%H:%M:%S'),
                "open": round(c.open, 5),
                "high": round(c.high, 5),
    ...
    async def get_candles_live2(
        self, asset: str, timeframe: Union[str, int], utc_offset: Optional[int] = 3
        ...
        if self._last_live_candle:
            c = self._last_live_candle
            adjusted_time = c.timestamp
            
            # --- تعديل التوقيت اللحظي ---
            if utc_offset is not None:
                from datetime import datetime, timedelta
                now_utc = datetime.utcnow()
                
                # حساب فرق الساعات بين الشمعة اللحظية والـ UTC الصافي
                current_wrong_offset = round((adjusted_time - now_utc).total_seconds() / 3600)
                
                # حساب الفرق بين التوقيت اللي راجع والتوقيت المطلوب
                adjustment_hours = current_wrong_offset - utc_offset
                
                # تطبيق الفرق
                if adjustment_hours != 0:
                    adjusted_time = adjusted_time - timedelta(hours=adjustment_hours)

            return {
                "timestamp": adjusted_time.strftime('%H:%M:%S'),
                "open": round(c.open, 5),
                "high": round(c.high, 5),
                "low": round(c.low, 5),
                "close": round(c.close, 5),
    ...
    async def get_candles_dataframe2(
        self,
        asset: str,
        timeframe: Union[str, int],
        count: int = 100,
        end_time: Optional[datetime] = None,
        utc_offset: Optional[int] = 3,
    ) -> pd.DataFrame:
        """
        Get historical candle data as DataFrame with bulletproof dynamic UTC offset
        """
        candles = await self.get_candles(asset, timeframe, count, end_time)
        ...
        if not df.empty:
            ...
            if utc_offset is not None:
                # 1. هنجيب التوقيت العالمي الفعلي دلوقتي
                now_utc = datetime.utcnow()
                
                # 2. هنجيب توقيت آخر شمعة في الداتا
                last_candle_time = df.index[-1]
                
                # 3. نحسب السيرفر مدينا توقيت فرق كام ساعة عن الـ UTC الصافي
                # بنقسم على 3600 عشان نحول الثواني لساعات وبنستخدم round عشان نتجاهل الكسور
                current_wrong_offset = round((last_candle_time - now_utc).total_seconds() / 3600)
                
                # 4. نحسب الفرق بين التوقيت اللي معاك والتوقيت اللي إنت طالبه
                adjustment_hours = current_wrong_offset - utc_offset
                
                # 5. نطرح الفرق ده من كل الشموع
                if adjustment_hours != 0:
                    df.index = df.index - pd.Timedelta(hours=adjustment_hours)
            ...
            if order_id in self._order_results:
                ...
                return {
                    "result": (
                        ...
                        else "lose" if result.status == OrderStatus.LOSE else "draw"
                    ),
    ...
    async def _request_candles(self, asset: str, timeframe: int, count: int, end_time: datetime):
        """Request real-time subscription AND historical candles from server using dynamic Server Time"""
        ...
        # 1. نبعت طلب التحديث اللحظي أولاً عشان نجس نبض السيرفر ونعرف توقيته
        live_data = {"asset": str(asset), "period": timeframe}
        live_msg = f'42{json.dumps(["changeSymbol", live_data])}'
        ...
        # 2. نصبر ثانية واحدة بس نستقبل فيها رسالة الـ history اللحظية عشان ناخد منها وقت السيرفر
        await asyncio.sleep(1.0)

        # 3. تحديد وقت السيرفر الفعلي
        end_ts = int(end_time.timestamp())
        cache_key = f"{asset}_{timeframe}"
        
        # لو المستخدم مش محدد وقت قديم، نستخدم توقيت سيرفر OTC المأخوذ من التحديث اللحظي
        if (datetime.now() - end_time).total_seconds() < 120:
            if cache_key in self._candles_cache and len(self._candles_cache[cache_key]) > 0:
                # ناخد وقت آخر شمعة السيرفر بعتها
                end_ts = int(self._candles_cache[cache_key][-1].timestamp.timestamp())
                if self.enable_logging:
                    logger.info(f"Using actual OTC server time: {end_ts}")

        # 4. نطلب الـ 100 شمعة بناءً على وقت السيرفر اللي جبناه
        history_data = {
            "asset": str(asset),
            "index": end_ts,
            "time": end_ts,
        ...
        history_msg = f'42{json.dumps(["loadHistoryPeriod", history_data])}'

        if self.enable_logging:
            logger.debug(f"Requesting {count} historical candles ending at {end_ts}")

        if self._is_persistent and self._keep_alive_manager:
            await self._keep_alive_manager.send_message(history_msg)
        else:
            await self._websocket.send_message(history_msg)

        try:
            # ننتظر لحد ما الشموع التاريخية توصل وتقفل الـ Future
    ...
    async def _on_json_data(self, data: Union[Dict[str, Any], List[Any]]) -> None:
        """Handle detailed order data and LIVE candles from JSON messages"""
        
# ==========================================
        # 1. التحديث اللحظي (Tick by Tick)
        # ==========================================
        if isinstance(data, list) and len(data) > 0 and isinstance(data[0], list) and len(data[0]) == 3:
            tick_data = data[0]
            if isinstance(tick_data[0], str) and isinstance(tick_data[1], (int, float)) and isinstance(tick_data[2], (int, float)):
                asset = tick_data[0]
                timestamp = float(tick_data[1])
                price = float(tick_data[2])
                
                for cache_key in list(self._candles_cache.keys()):
                    if cache_key.startswith(f"{asset}_"):
                        period = int(cache_key.split("_")[-1])
                        aligned_ts = timestamp - (timestamp % period)
                        candle_time = datetime.fromtimestamp(aligned_ts)
                        
                        if self._candles_cache[cache_key]:
                            last_candle = self._candles_cache[cache_key][-1]
                            if candle_time == last_candle.timestamp:
                                updated_candle = Candle(
                                    ...
                                    close=price,
                                    volume=last_candle.volume + 1.0,
                                    asset=last_candle.asset,
                                    timeframe=last_candle.timeframe
                                )
                                self._candles_cache[cache_key][-1] = updated_candle
                                # ----- السطر الجديد هنا -----
                                self._last_live_candle = updated_candle
                            elif candle_time > last_candle.timestamp:
                                new_candle = Candle(
                                    timestamp=candle_time, open=price, high=price, low=price, close=price, 
                                    volume=1.0, asset=asset, timeframe=period
                                )
                                self._candles_cache[cache_key].append(new_candle)
                                # ----- السطر الجديد هنا -----
                                self._last_live_candle = new_candle
                        else:
                            new_candle = Candle(
                                timestamp=candle_time, open=price, high=price, low=price, close=price, 
                                volume=1.0, asset=asset, timeframe=period
        ...
        # ==========================================
        # 2. الشموع التاريخية الكاملة (History Period)
        # ==========================================
        if "data" in data and isinstance(data["data"], list) and "asset" in data and "period" in data:
            asset = data["asset"]
            period = data["period"]
            ...
            if len(candle_data) > 0:
                ...
                existing_candles = {c.timestamp: c for c in self._candles_cache[cache_key]}
                ...
                for item in candle_data:
                    try:
                        ...
                        candle = Candle(
                            timestamp=timestamp, open=open_p, high=high_p, low=low_p, close=close_p, 
                            volume=volume, asset=asset, timeframe=period
                        )
                        existing_candles[timestamp] = candle
                    except Exception as e:
                        if self.enable_logging:
                            logger.error(f"Error parsing period candle: {e}")
                
                updated_candles = sorted(list(existing_candles.values()), key=lambda x: x.timestamp)
                self._candles_cache[cache_key] = updated_candles
                ...
                if hasattr(self, "_candle_requests") and request_id in self._candle_requests:
                    ...
                    if not future.done():
                        future.set_result(updated_candles)
                        if self.enable_logging:
                            logger.success(f"Successfully loaded {len(updated_candles)} period candles for {asset}")
                return

        # ==========================================
        # 3. الشموع التعويضية (History Ticks)
        # ==========================================
        if "history" in data and isinstance(data["history"], list) and "asset" in data and "period" in data:
            asset = data["asset"]
            period = data["period"]
            ticks = data["history"]
            ...
            if ticks and len(ticks) > 0:
                ...
                for tick in ticks:
                    try:
                        ...
                        if candle_time in existing_candles:
                            old_c = existing_candles[candle_time]
                            # التعديل السحري: بناء شمعة جديدة وتحديث القديمة عشان نتخطى حماية الـ Immutable
                            updated_c = Candle(
                                timestamp=old_c.timestamp,
                                open=old_c.open,
                                high=max(old_c.high, price),
                                low=min(old_c.low, price),
                                close=price,
                                volume=old_c.volume + 1.0,
                                asset=old_c.asset,
                                timeframe=old_c.timeframe
                            )
                            existing_candles[candle_time] = updated_c
                        else:
                            new_candle = Candle(
                                timestamp=candle_time, open=price, high=price, low=price, close=price, 
                                volume=1.0, asset=asset, timeframe=period
                            )
                            existing_candles[candle_time] = new_candle
                    except Exception as e:
                        if self.enable_logging:
                            logger.error(f"Error parsing history tick: {e}")
                        
                self._candles_cache[cache_key] = sorted(list(existing_candles.values()), key=lambda x: x.timestamp)
                return
    ...
    async def _on_candles_received(self, data: Any) -> None:
        """Handle historical candles data received from loadHistoryPeriod"""
        if self.enable_logging:
            logger.info("Historical candles received from server")
                ...
                for request_id, future in list(self._candle_requests.items()):
                    if not future.done():
                        ...
                        if len(parts) >= 2:
                            ...
                            parsed_candles = self._parse_stream_candles({"data": candle_data}, asset, timeframe)
                            
                            if parsed_candles:
                                if cache_key not in self._candles_cache:
                                    self._candles_cache[cache_key] = []
                                    
                                # دمج الـ 100 شمعة مع الكاش بدون ما نمسح التحديثات اللحظية
                                existing_candles = {c.timestamp: c for c in self._candles_cache[cache_key]}
                                for c in parsed_candles:
                                    existing_candles[c.timestamp] = c
                                ...
                                future.set_result(updated_candles)
                                if self.enable_logging:
                                    logger.success(f"Resolved historical request: {len(parsed_candles)} candles loaded")
            except Exception as e:
                if self.enable_logging:
                    logger.error(f"Error processing historical candles: {e}")
    ...
    async def _handle_candles_stream(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if self.enable_logging:
                logger.info(f"Processing candle stream for {asset} ({period}s)")

            # 1. التحديث اللحظي (Live Candle) بييجي في مفتاح "data"
            # ده مش بنقفل بيه الانتظار (Future)، إحنا بس بنحدث بيه الكاش في صمت
            if "data" in data:
                live_candles = self._parse_stream_candles({"data": data["data"]}, asset, period)
                if live_candles:
                    if cache_key not in self._candles_cache:
                        self._candles_cache[cache_key] = []
                    
                    # دمج ذكي عشان التحديث اللحظي مايمسحش القديم
                    existing_candles = {c.timestamp: c for c in self._candles_cache[cache_key]}
            ...
            # 2. الشموع التاريخية كاملة بتيجي في مفتاح "candles"
            # دي اللي بنقفل بيها الانتظار عشان دالة get_candles تكمل شغلها
            if "candles" in data:
                if hasattr(self, "_candle_requests") and request_id in self._candle_requests:
                    future = self._candle_requests[request_id]
                    if not future.done():
                        hist_candles = self._parse_stream_candles({"candles": data["candles"]}, asset, period)
                        if hist_candles:
                            future.set_result(hist_candles)
                            if self.enable_logging:
                                logger.success(f"Resolved candle request for {asset} with {len(hist_candles)} candles")
